...
(define (domain exploding-blocksworld)
  ...
  (:action pick-up
   ...
   :precondition (and (handempty) (clear ?b1) (on ?b1 ?b2) (no-destroyed ?b1))
   :effect (and (increase (total-cost) 1) (holding ?b1) (clear ?b2) (not (handempty)) (not (on ?b1 ?b2)) (not (clear ?b1)))
  )

  (:action pick-up-from-table
   :parameters (?b - block)
   :precondition (and (handempty) (clear ?b) (ontable ?b) (no-destroyed ?b))
   :effect (and (increase (total-cost) 1) (holding ?b) (not (handempty)) (not (ontable ?b)) (not (clear ?b)))
  )

  (:action put-down-no-detonation
   :parameters (?b - block)
   :precondition (and (holding ?b) (no-destroyed-table) (not (no-detonated ?b)))
   :effect (and (increase (total-cost) 1) (handempty) (clear ?b) (ontable ?b) (not (holding ?b)))
  )

  (:action put-down-detonation
   :parameters (?b - block)
   :precondition (and (holding ?b) (no-destroyed-table) (no-detonated ?b))
   :effect (and (increase (total-cost) 1) (handempty) (ontable ?b) (not (holding ?b)) (clear ?b)
  ...
  (:action put-on-block-no-detonation
   ...
   :effect (and (increase (total-cost) 1) (handempty) (on ?b1 ?b2) (not (holding ?b1)) (not (clear ?b2)) (clear ?b1))
  )
